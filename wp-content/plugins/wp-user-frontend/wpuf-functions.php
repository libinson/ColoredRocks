...
function wpuf_notify_post_mail( $user, $post_id ) {
    ...
    $subject = sprintf( __( '[%s] New Essay Submission' ), $blogname );

    $msg = sprintf( __( 'A new essay has been submitted on %s' ), $blogname ) . "\r\n\r\n";
    $msg .= sprintf( __( 'Username : %s' ), $user->user_login ) . "\r\n";
    $msg .= sprintf( __( 'From : %s' ), $user->display_name ) . "\r\n";
    $msg .= sprintf( __( 'Author Email : %s' ), $user->user_email ) . "\r\n";
    $msg .= sprintf( __( 'Essay Title : %s' ), get_the_title( $post_id ) ) . "\r\n";
    $msg .= sprintf( __( 'Submission Date : %s' ), get_the_date( 'm-d-Y', $post_id ) ) . "\r\n";
   ...
   // $msg .= sprintf( __( 'Edit Link : %s' ), admin_url( 'post.php?action=edit&post=' . $post_id ) ) . "\r\n";
    ...
    $user_headers = sprintf( "From: %s <%s>\r\n", $blogname, $user_to );
    $user_subject = sprintf( __( '[%s] Essay Submitted Successfully' ), $blogname );

	$user_msg = "Thank you!  \r\n Your Colored Rocks essay has been submitted. Good luck. \r\n Finalists will be announced in February.";
	
	wp_mail( $user_to, $user_subject, $user_msg, $user_headers );
}
