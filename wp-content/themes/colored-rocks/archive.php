...
<!-- tpl: archive -->
	<?php if (have_posts()) { ?>
        <header class="page-header">
	        <h1 class="page-title">
		        <?php if (is_day()) { ?>
			        <?php printf(__('Daily Archives: %s', 'colored_rocks'), '<span>' . get_the_date() . '</span>'); ?>
		        <?php } elseif (is_month()) { ?>
			        <?php printf(__('Monthly Archives: %s', 'colored_rocks'), '<span>' . get_the_date('F Y') . '</span>'); ?>
		        <?php } elseif (is_year()) { ?>
			        <?php printf(__('Yearly Archives: %s', 'colored_rocks'), '<span>' . get_the_date('Y') . '</span>'); ?>
		        <?php } else { ?>
			        <?php _e('Essay Archives', 'colored_rocks'); ?>
		        <?php } ?>
	        </h1>
        </header>
	...
	<?php } else { 
	    colored_rocks_no_results(__('Apologies, but no results were found for the requested archive. Perhaps searching will help find a related post.', 'colored_rocks'));
	} ?>
